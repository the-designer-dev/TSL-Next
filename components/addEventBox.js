...
function AddEventBox(props) {
    ...
    async function AddNewEvent(e) {
        ...
        if (!isOverlapping(copyBlackoutDates, { title: name, start: props.range ? props.range[0] : start.format('YYYY-MM-DD'), end: props.range ? props.range[1] : end.format('YYYY-MM-DD'), roomType: formats[0], refundableRates: refundableRates, nonRefundableRates: nonRefundableRates, quantity: quantity })) {
            await axios({
                ...
                data: { hotel: currentHotel, title: name, start: props.range ? props.range[0] : start.format('YYYY-MM-DD'), end: props.range ? props.range[1] : end.format('YYYY-MM-DD'), roomType: formats[0], refundableRates: refundableRates, nonRefundableRates: nonRefundableRates, quantity: quantity }
            }).then(res => {
    ...
    function editEvent(e) {
        ...
        const res = axios({
            ...
            data: { oldObj, newObj, hotel: currentHotel }
            ...
            <form onSubmit={(e) => { if (props.name) { editEvent(e) } else { AddNewEvent(e) } }}>
                <Grid container spacing={2}>
                    ...
                    <Grid container item xs={12} spacing={1}>
                        ...
                        <Grid item xs={12}><StyledTextField InputLabelProps={{ shrink: false }}
                            label="" value={name} onChange={e => setName(e.target.value)} fullWidth /></Grid>
                    </Grid>
                    <Grid container item xs={12} spacing={1}>
                        ...
                        <Grid item xs={12}>
                            <Box sx={{ width: '100%' }}>
                                ...
                                >
                                    <ToggleButton sx={{ '&.MuiToggleButton-root': { '&.Mui-selected': { backgroundColor: 'button.main', color: "#FFF" } }, }} value="suite" aria-label="bold">
                                        <Typography variant='p'>Suite</Typography>
                                    </ToggleButton>
                                    <ToggleButton sx={{ '&.MuiToggleButton-root': { '&.Mui-selected': { backgroundColor: 'button.main', color: "#FFF" } } }} value="executive" aria-label="italic">
                                        <Typography variant='p'>Executive</Typography>
                                    </ToggleButton>
                                    <ToggleButton sx={{ '&.MuiToggleButton-root': { '&.Mui-selected': { backgroundColor: 'button.main', color: "#FFF" } } }} value="standard" aria-label="italic">
