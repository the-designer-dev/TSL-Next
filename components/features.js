...
export default function Features() {
  ...
  function searchIcon(e){
    axios.get(`https://api.iconfinder.com/v3/icons/search?query=${e.target.value}&count=30`, {
      params: {
    ...
    .then((res) => {
      ...
      console.log(res.data.icons.map((el) => (el.raster_sizes[0].formats[0].preview_url)))
    })
    .catch((error) => {
      console.error(error)
    })
          ...
          <Box sx={{width:'100%'}}>
            ...
            <Grid container spacing={2}>       
                ...
                <Grid container item xs={12} sm={6} alignItems='center'>
                    ...
                    <Grid container item xs={2} justifyContent='flex-end'><Checkbox checked={amenitiesRedux.find((ele) => ele.service_name === el.service_name)} onChange={(e) => addAmenity(e.target.checked,el.service_icon , el.service_name ,el.service_description)}/></Grid>
                </Grid>))}
            </Grid>
          </Box>
...
<Grid container spacing={2}>
    ...
    {facilities.map((el) =>(
                <Grid container item xs={12} sm={6} alignItems='center'>
                    ...
                    <Grid container item xs={2} justifyContent='flex-end'><Checkbox checked={facilitiesRedux.find((ele) => ele.service_name === el.service_name)} onChange={(e) => addFacility(e.target.checked,el.service_icon , el.service_name ,el.service_description)}/></Grid>
                </Grid>))}
</Grid>
</Box>
      </TabPanel>
      ...
      <Box sx={{width:'100%'}}>
        ...
        <Grid container spacing={2}>
            <Grid item xs={12}>
              <Typography fontWeight={600} variant='p'>Add more services:</Typography>
            </Grid>
            <Grid container item xs={12}>
              <Grid item xs={12} sm={6}>
              <Typography  fontWeight={300} variant='p'>Service Name:</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
              <StyledTextField onChange={(e) => setServiceName(e.target.value)} fullWidth/>
              </Grid>
            </Grid>
            <Grid container item xs={12}>
              <Grid item xs={12} sm={6}>
              <Typography  fontWeight={300} variant='p'>Service Description:</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
              <StyledTextField onChange={(e) => setserviceDescription(e.target.value)} fullWidth/>
              </Grid>
            </Grid>
            <Grid  container item xs={12}>
              <Grid item xs={12} sm={6}>
              <Typography fontWeight={300} variant='p'>Service Icon:</Typography>
              </Grid>
              
              <Grid item xs={11} sm={5}>
         <StyledTextField  placeholder='Search' fullWidth onChange={e => searchIcon(e)}  /> 
            </Grid>
            <Grid item xs={1} sm={1}>
              {selectedIcon? <img style={{ paddingLeft:'10px',height:'56px'}} src={selectedIcon}/>:''}
              </Grid>
            <Grid container justifyContent='flex-end' item xs={12} >
              ...
              <Grid item xs={12} sm={6}>
              {icon.length> 0 ?icon.map((el) => ( <Button onClick={()=>{setSelectedIcon(el.raster_sizes[0].formats[0].preview_url)}}><img style={{height:'20px'}} src={el.raster_sizes[0].formats[0].preview_url}/></Button>)):''}
              </Grid>
            </Grid>
            <Grid container item justifyContent='flex-end' >
            <Grid sx={{paddingTop:'16px' ,textAlign:'right'}} item xs={12} sm={6}>  
              <StyledButton onClick={() => addService()}>Add Service</StyledButton>
            </Grid>
            </Grid>
            </Grid>
