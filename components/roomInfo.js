    ...
    return (
        <Box>
                ...
                <Grid item xs={12} sm={12} md={6} >
                    ...
                    <form>
                    <FormControl>
                    <Grid xs={12} alignItems='center' spacing={2} direction='row' justifyContent='center' container item><Grid justifyContent='flex-end' alignContent='center' container item xs={4}> <Button onClick={e => {if(rooms-1 >= 0){setRooms(rooms -1 ); setCurrRoomPrice(el.blackout_dates?el.blackout_dates.nonrefundable_rates : el.roomnonrefundprice)}}}  sx={{ borderRadius:'100%' , backgroundColor:'transparent' , color:'button.main'  , border:'1px solid' , minWidth:'40px !important'}}>-</Button></Grid> <Grid container item sx={{textAlign:'center'}} xs={4}> <StyledTextField value={rooms} onChange={e => {if(parseInt(el.blackout_dates? el.blackout_dates.quantity : el.roomqty) >= e.target.value && e.target.value >= 0) {setRooms(e.target.value); setCurrRoomPrice(el.blackout_dates?el.blackout_dates.nonrefundable_rates : el.roomnonrefundprice)} }} defaultValue="0" value={rooms} id="outlined-name"  size='small' sx={{width:'100%'}} type='number' /></Grid> <Grid justifyContent='flex-start' alignContent='center' container item xs={4}> <Button onClick={e => {if(rooms+1 <= parseInt(el.blackout_dates? el.blackout_dates.quantity : el.roomqty)){setRooms(rooms +1 ); setCurrRoomPrice(el.blackout_dates?el.blackout_dates.nonrefundable_rates : el.roomnonrefundprice)}}} sx={{ borderRadius:'100%' , backgroundColor:'transparent' , color:'button.main'  , border:'1px solid' , minWidth:'40px !important'}}>+</Button></Grid></Grid>
                    </FormControl>
                    </form>
                    </Grid>
