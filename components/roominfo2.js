...
function RoomInfo2(props) {
  ...
  const [extraBed, setExtraBed] = useState({
    extra_bed_qty: 0,
    extra_bed_rates:
      el.extraBeds.length > 0 ? el.extraBeds[0].extra_bed_rates : 0,
  });
  ...
  const handleFormat = (event, newFormats) => {
    ...
    setCurrRoomPrice(
      el.blackout_dates
        ? newFormats === "refundable"
          ? el.blackout_dates.nonrefundable_rates
          : el.blackout_dates.refundable_rates
        : newFormats === "refundable"
        ? el.roomrefundprice
        : el.roomnonrefundprice
    );
  };
  ...
  const calculatePrice = () => {
    ...
    //   });
    temp_price =
      temp_price + extraBed.extra_bed_qty * extraBed.extra_bed_rates * days;
    ...
    >
      ...
      <Grid container columns={18} spacing={4}>
        <Grid container item xs={18} sm={18} md={7}>
          <Box sx={{ width: "100%" }}>
            {el.images.length > 0 ? (
              <SmallCarouselWithThumbnail images={el.images} />
            ) : (
              ""
            )}
          </Box>
              ...
              <Grid item xs={18}>
                <Box>
                  <Typography fontWeight={600} fontSize={12}>
                    {bedsAvailable.map(
                      (el) => `${el.occurrence} ${el.bedName} Sized Bed `
                    )}
                  </Typography>
                </Box>
              </Grid>
              <Grid container item sx={{ lineHeight: "14px" }}>
                ...
                {el.room_includes.map((ele) => (
                  <Grid container item xs={18} alignItems="center" spacing={1}>
                    <Grid item>
                      <CheckIcon
                        sx={{ fontSize: "16px", color: "button.main" }}
                      />
                    </Grid>
                    <Grid item>
              ...
              <Grid container item xs={18} spacing={1}>
                <Grid item xs={18}>
                  ...
                  >
                    <ToggleButton
                      ...
                      aria-label="bold"
                    >
                      <Typography fontSize={10} fontWight={600} variant="p">
                        Refundable
                      </Typography>
                    </ToggleButton>
                    <ToggleButton
                ...
                {formats === "refundable" ? (
                  <Grid item xs={18} sx={{ lineHeight: "14px" }}>
                    {el.free_cancellation_days ? (
                      <>
                        <Typography fontSize={12} variant="p">
                          - Free cancellation before{" "}
                          {moment(hotelQuery.checkIn)
                            .subtract(el.free_cancellation_days, "days")
                            .format("DD-MM-YYYY")}
                        </Typography>
                        <br />
                      </>
                    ...
                    {el.cancelled_within_days ? (
                      <>
                        <Typography fontSize={12} variant="p">
                          - Penalty of PKR {currRoomPrice * el.applied_for_days}{" "}
                          Within{" "}
                          {moment(hotelQuery.checkIn)
                            .subtract(el.cancelled_within_days, "days")
                            .format("DD-MM-YYYY")}{" "}
                          to {moment(hotelQuery.checkIn).format("DD-MM-YYYY")}
                        </Typography>
                        <br />
                      </>
                    ) : (
                    ...
                    {el.no_show ? (
                      <Typography fontSize={12} variant="p">
                        - No show cancellation fee of {el.no_show}% of your bill
                      </Typography>
                    ) : (
                      ""
                    )}
            ...
            <Grid container item xs={7} spacing={2}>
              ...
              <Grid container item sx={{ lineHeight: "14px" }} sm={18}>
                ...
                <Typography
                  ...
                  color={"#a5a5a5"}
                >
                  (PKR {el.extraBeds[0].extra_bed_rates} per bed)
                </Typography>
                <form>
                  <FormControl>
                      ...
                      >
                        <StyledTextField
                          ...
                          onChange={(e) =>
                            setExtraBed({
                              ...extraBed,
                              extra_bed_qty: e.target.value,
                            })
                          }
                          id="outlined-name"
                          value={extraBed.extra_bed_qty}
                          sx={{
                            width: "100%",
                            height: "28px",
                            boxSizing: "border-box !important",
        ...
        <Grid
          ...
          sm={6}
          md={3}
          sx={{ paddingLeft: "16px !important", lineHeight: "1" }}
          spacing={3}
        >
          <Grid
            container
          ...
          >
            ...
            <Grid item xs={18}>
              <Typography fontWeight={100} fontSize={10} color={"#a5a5a5"}>
          ...
          >
            ...
            <Grid item xs={18}>
              <Typography fontWeight={500} fontSize={12}>
                {" "}
                {hotelQuery
                  ? `${moment(hotelQuery.checkIn).format(
                      "DD-MM-YYYY"
                    )} - ${moment(hotelQuery.checkOut).format("DD-MM-YYYY")}`
                  : ""}
              </Typography>
            </Grid>
            <Grid item xs={18}>
          ...
          <Grid item xs={18}>
            <StyledButton onClick={() => submitData(data.id, el.id)} fullWidth>
              Book Now
            </StyledButton>
          </Grid>
        </Grid>
