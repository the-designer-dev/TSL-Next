...
function OrderDetails(props) {
  ...
  useEffect(async () => {
    ...
    if (order) {
      const vendorVar = await axios({
        method: "GET",
        url: `${API_URL}/orders_vendor/${order}`,
  ...
  var childBooking =
    orderDetails &&
    orderDetails.child_booking.map((child) => {
      return {
        dob: child.dob,
        name: child.name,
        cat: "child",
  ...
  var adultBooking =
    orderDetails &&
    orderDetails.adult_booking.map((adult) => {
      return {
        dob: "",
        name: adult.name,
        cat: "adult",
  ...
  const cancelOrder = () => {
    axios
      .get(`${API_URL}/cancel_order/${order}`, {
        headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` },
      })
      .then((res) => {
...
OrderDetails.getLayout = function getLayout(OrderDetails) {
  return <VendorLayout>{OrderDetails}</VendorLayout>;
};
