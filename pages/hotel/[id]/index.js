...
export default function Hotel({resData , reqData,id}) {
    ...
    useEffect(async() => {
        const fetchRestaurants = async () => {
          await axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/restaurant.json?type=poi&proximity=${data.longitude},${data.latitude}&access_token=${mapboxApiKey}&limit=5`).then((res) => {console.log(res.data.features); setRestaurant(res.data.features)}).catch((err) => console.log({ err }));};
          fetchRestaurants();
        const fetchAttractions = async () => {
          await axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/outdoors.json?type=poi&proximity=${data.longitude},${data.latitude}&access_token=${mapboxApiKey}&limit=5`).then((res) => {console.log(res.data.features); setAttractions(res.data.features)}).catch((err) => console.log({ err }));};
          fetchAttractions();
        const fetchHospitals = async () => {
          await axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/hospital.json?type=poi&proximity=${data.longitude},${data.latitude}&access_token=${mapboxApiKey}&limit=5`).then((res) => {console.log(res.data.features); setHospitals(res.data.features)}).catch((err) => console.log({ err }));};
          fetchHospitals();
        const fetchAirports = async () => {
          await axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/airport.json?type=poi&proximity=${data.longitude},${data.latitude}&access_token=${mapboxApiKey}&limit=5`).then((res) => {console.log(res.data.features); setAirports(res.data.features)}).catch((err) => console.log({ err }));};
          fetchAirports();
    ...
    return (
        <StyledContainer sx={{height:'100%'}}>
            ...
            <Grid  item xs={12}  md={4}>
                <Box sx={{backgroundColor:'background.main' , height:'100%' , maxWidth:{ md:'312px' , sm:'100%'} , padding:'20px 20px 0px 20px' , borderRadius:'8px'}}>
                ...
                (<Grid container item  alignItems='center'  xs={6} sm={3} md={6}><Grid item xs={2}><Box sx={{display:'flex' , justifyContent:'flex-end'}}><img src={el.service_icon}/></Box> </Grid><Grid item xs={10}> <Typography sx={{paddingLeft:'10px'}}variant='p'>{el.service_name}</Typography></Grid></Grid>)
                )}
                </Grid>
            ...
            <Grid item xs={12}>
            <Box sx={{  width: '95.5%' , margin:'auto' ,bgcolor:'background.main' , borderRadius:'8px'}}>
            ...
            <TabPanel value={value2} index={0} >
              <Box className='poi_box' sx={{maxHeight:'200px' , overflow:'scroll' , overflowX:'hidden'}}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid justifyContent='left' direction='row' alignItems='center' container spacing={1} >
      ...
      <Accordion sx={{'&.MuiAccordion-root':{backgroundColor:'transparent' , boxShadow:'none',backgroundImage:'none'}}}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid justifyContent='left' direction='row' alignItems='center' container spacing={2} >
            ...
            {el.properties.category ?el.properties.category.split(',').map((el,index) =>
            (
           index <4?     
          <Grid item spacing >
            <Box sx={{backgroundColor:'button.main' , justifyContent:'center' , padding:'5px' , borderRadius:'20px' , display:'flex' , textAlign:'center'  }}>
      ...
      <Accordion sx={{'&.MuiAccordion-root':{backgroundColor:'transparent' , boxShadow:'none',backgroundImage:'none'}}}>
        ...
        <Grid container spacing={2}>
          ...
          <Typography fontSize='12px' fontWeight={400} variant='p'>
            ...
            <Map locations={{longitude : data.longitude ,latitude : data.latitude}} restaurants={restaurant} airports={airports} hospitals={hospitals} attractions={attractions} />
            </Grid>
            </Grid>
            </Box>
            </TabPanel>
