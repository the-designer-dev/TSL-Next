...
function Book(props) {
    ...
    useEffect(()=>{
        var index =1
      data != null && data != undefined?  setRows(rows => [...rows , {id: index , sign:'>' , particular: data.rooms.find((el) => el.id === parseInt(room)).roomname , quantity : 'x'+roomQuantity , 'unit price' : data.rooms.find((el) => el.id === parseInt(room)).roomnonrefundprice , price: data.rooms.find((el) => el.id === parseInt(room)).roomnonrefundprice*roomQuantity } ])
      : setRows([])
  ...
  <StyledContainer square={true}>
      ...
      <Grid container spacing={2} >
        ...
        <Grid container item xs={4} >
            <Grid container item spacing={2} >
                ...
                <Grid item xs={12}>
                <Typography  variant='p'>This option includes:</Typography>
                </Grid>

                  {data.rooms.find((el) => el.id === parseInt(room)).room_includes.map((el) => (  
                <Grid container item xs={12} sm={6} md={3} alignItems='center' spacing={1}>
                <Grid item xs={3}><CheckIcon sx={{color:'button.main'}}/></Grid><Grid item xs={9}><Typography  variant='p'>{el.service_name}</Typography></Grid>
                </Grid>))}
            </Grid>
        </Grid>
        <Grid container item xs={5} >
            <Grid container item spacing={2} >
                ...
                {data.rooms.find((el) => el.id === parseInt(room)).amenities.map((el , index) => (  
                 index <4?   
                <Grid container item xs={12} sm={6} md={3} alignItems='center' spacing={1}>
                <Grid item xs={3}>{el.service_icon}</Grid><Grid item xs={9}><Typography  variant='p'>{el.service_name}</Typography></Grid>
                </Grid>:<></>))}
                  {data.rooms.find((el) => el.id === parseInt(room)).amenities.map((el , index) => (
                    index>3 && index <7?  
                <Grid container item  xs={12} sm={6} md={3} alignItems='center' spacing={1}>
    ...
    <Grid container spacing={2}  sx={{padding:'16px 0px'}}>
      {adultCards.map((el) => <Grid item xs={12} sm={4} md ={3} lg={2} ><AdultCard num={el}/></Grid>  )}
      {childCards.map((el) =>  <Grid item xs={12} sm={4} md ={3} lg={2} ><ChildCard num={el}/></Grid> )}
      </Grid>
      <Grid container spacing={2}>
        ...
        <Grid item ><StyledButton sx={{marginTop:'20px'}}>Check Out</StyledButton></Grid>
        </Grid>
        </Box></Grid>
      </Grid>
      </StyledContainer>            
